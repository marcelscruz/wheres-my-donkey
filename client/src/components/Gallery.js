...
export class Gallery extends Component {
  ...
  fetchPhotos = async () => {
    ...
    try {
      ...
      addPhotos(photos) // Add fetched photos in the store
      setNextPage(page + 1) // Set next group of photos to be fetched
  ...
  renderPhotos = () => {
    ...
    return (
      <div>
        {photos.map(photo => {
          return (
            <Photo key={photo.id} photo={photo} toggleModal={toggleModal} />
          )
        })}
      </div>
    )
